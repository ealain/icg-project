...
void main() {
    ...
    vec2 position = uv + vec2(movement.x,movement.y); 
    ...
    //texture 
    vec3 sandTex = texture(sandTex, scale * uv).rgb;
    vec3 grassTex = texture(grassTex, scale * uv).rgb;
    vec3 rockTex = texture(rockTex, scale * uv).rgb;
    vec3 snowTex = texture(snowTex, scale * uv).rgb;
